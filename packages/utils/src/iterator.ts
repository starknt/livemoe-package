...
export namespace Iterator {
  ...
  export function from<T>(
    elements: Iterator<T> | T[] | undefined,
  ): Iterator<T> {
    if (!elements)
      return empty()
  ...
  export function concat<T>(...iterators: Array<Iterator<T>>): Iterator<T> {
    let i = 0
...
export class MappedNavigator<T, R>
  extends MappedIterator<T, R>
