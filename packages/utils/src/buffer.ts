...
export class VSBuffer {
  ...
  slice(start?: number, end?: number): VSBuffer {
    ...
    return new VSBuffer(this.buffer.subarray(start || 0, end))
  }
...
export function readUInt32LE(source: Uint8Array, offset: number): number {
  return (
    ((source[offset + 0] << 0) >>> 0) | ((source[offset + 1] << 8) >>> 0) | ((source[offset + 2] << 16) >>> 0) | ((source[offset + 3] << 24) >>> 0)
  )
}
...
export function isVSBufferReadableStream(
  obj: any,
): obj is VSBufferReadableStream {
 ...
 * @example
 */
export function toVSBufferReadableStream(
...
class VSBufferWriteableStreamImpl implements VSBufferWriteableStream {
  ...
  private readonly listeners = {
    data: [] as Array<(chunk: VSBuffer) => void>,
